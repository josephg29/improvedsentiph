  ...
  return (
    <ConfirmationDialog
      ...
      message={
        isCleanupIntent ? (
          <>
            Cleanup <strong>{pendingDeleteTerminal.agentName}</strong> and delete the agent
            session metadata.
          </>
        ) : isCloseIntent ? (
          <>
            Close <strong>{pendingDeleteTerminal.agentName}</strong> and terminate its active
            terminal session.
          </>
        ) : (
          <>
            Delete <strong>{pendingDeleteTerminal.agentName}</strong> and terminate all of its
            active sessions.
          </>
        )
      }
    ...
    >
      <dl className="delete-confirm-details">
        <div>
          <dt>Name</dt>
          <dd>{pendingDeleteTerminal.agentName}</dd>
        </div>
        <div>
          <dt>ID</dt>
